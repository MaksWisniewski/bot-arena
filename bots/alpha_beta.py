...
class MinMax_Bot(Bot):
    ...
    def min_max_search(self, game: Game, depth: int, _player: str, alpha = float("-inf"), beta = float("inf")) -> float:

        if depth == 0 or game.is_win() is not None:
            return self.evaluate(game)

        __maximizing_player = _player == self.side
        legal_moves = game.get_legal_moves(_player)

        if __maximizing_player:
            value = float("-inf")
            for move in legal_moves:
                ...
                value = max(value, self.min_max_search(game_cp, depth - 1, negate_player(_player), alpha, beta))
            ...
            for move in legal_moves:
                ...
                value = min(value, self.min_max_search(game_cp, depth - 1, negate_player(_player), alpha, beta))
    ...
    def simulate(self, _depth = 3) -> str:
        ...
        print(f"alfa beta: best move: {best_move}", file=sys.stderr)
        return best_move
